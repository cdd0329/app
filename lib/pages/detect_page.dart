...
class _DetectPageState extends State<DetectPage> {
  ...
  Future<void> _detect() async {
    ...
    try {
      ...
      try {
        ...
        final imgPath = imgFile.path;
        await AppDatabase.insertRecord(DetectionRecord(
          id: now, imagePath: imgPath,
      ...
      var rawJson = resp.body.length > 300 ? resp.body.substring(0, 300) : resp.body;
      if (mounted) setState(() { _dets = list; _hasRun = true; _isBusy = false;
        _dbg = '${list.length}目标 ${sw.elapsedMilliseconds}ms | $rawJson'; });
    } catch (e) {
  ...
  Widget _enhSlider(String l, double v, double min, double max, ValueChanged<double> cb, String d) {
    return Row(children: [
      ...
      body: Column(children: [
        ...
        if (_dbg != null)
          Container(width: double.infinity, padding: const EdgeInsets.all(8),
              ...
              child: Text(_dbg!, style: const TextStyle(fontSize: 11, fontFamily: 'monospace'))),
          ...
          child: Column(children: [
            InkWell(onTap: () => setState(() => _enhExpanded = !_enhExpanded),
                child: Padding(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
                    child: Row(children: [
                      ...
                      Text('图片增强', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
                      const Spacer(),
            ...
            if (_enhExpanded) ...[const Divider(height: 1),
              Padding(padding: const EdgeInsets.fromLTRB(14, 4, 14, 8), child: Column(children: [
                _enhSlider('缩放', _scale, 0.5, 2.0, (v) => _scale = v, '${_scale.toStringAsFixed(1)}x'),
                _enhSlider('裁剪', _crop, 0.4, 1.0, (v) => _crop = v, '${(_crop * 100).toInt()}%'),
                _enhSlider('旋转', _rotate, -45, 45, (v) => _rotate = v, '${_rotate.toInt()}°'),
                _enhSlider('亮度', _brightness, -0.5, 0.5, (v) => _brightness = v, _brightness.toStringAsFixed(2)),
                _enhSlider('对比度', _contrast, -0.5, 0.5, (v) => _contrast = v, _contrast.toStringAsFixed(2)),
                _enhSlider('饱和度', _saturation, 0, 2, (v) => _saturation = v, '${_saturation.toStringAsFixed(1)}x'),
                Row(children: [
                  TextButton.icon(
                    onPressed: () => setState(() => _showOriginal = !_showOriginal),
                    icon: Icon(_showOriginal ? Icons.toggle_on : Icons.compare, size: 16),
                    label: Text(_showOriginal ? '恢复' : '对比', style: TextStyle(fontSize: 12)),
                    style: TextButton.styleFrom(foregroundColor: Colors.grey.shade600),
                  ),
                  const Spacer(),
                  TextButton.icon(
                    onPressed: () { _scale = 1; _crop = 1; _rotate = 0; _brightness = 0; _contrast = 0; _saturation = 1; setState((){}); },
